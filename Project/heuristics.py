...
def calcMD(matrix):
    distance = 0
    goalCoords = getGoalCoordinates(matrix)
...
def calcHeuristic(method, matrix):
    """
        Inputs
    ...
    if method == 'MD':
        return calcMD(matrix)
    if method == 'LC':
        return calcLC(matrix)
...
eightMatrix = [(1,2,6),
               (4,5,3),
               (8,7,0)]
fifteenMatrix = [(2,3,1,8),
                 (5,7,6,4),
                 (12,11,10,9),
                 (15,14,13,0)]

print("MD1",calcHeuristic('MD', eightMatrix))
print("LC1",calcHeuristic('LC', eightMatrix))
print("MD2",calcHeuristic('MD', fifteenMatrix))
print("LC2",calcHeuristic('LC', fifteenMatrix))
